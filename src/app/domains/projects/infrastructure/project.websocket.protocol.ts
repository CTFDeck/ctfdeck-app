...
export function serializeProjectCreate(name: string, description: string, messageId: string): Uint8Array {
  ...
  let offset = 0;
  buffer[offset] = MessageType.ProjectCreate;
  offset += 1;
...
export function serializeProjectUpdate(
  ...
  let offset = 0;
  buffer[offset] = MessageType.ProjectUpdate;
  offset += 1;
  buffer.set(uuidToBytes(messageId), offset);
  offset += 16;
  buffer.set(uuidToBytes(projectId), offset);
  offset += 16;
  view.setInt32(offset, nameBytes.length, true);
  offset += 4;
  buffer.set(nameBytes, offset);
...
export function serializeProjectAddFolder(
  ...
  let offset = 0;
  buffer[offset] = MessageType.ProjectAddFolder;
  offset += 1;
  buffer.set(uuidToBytes(messageId), offset);
  offset += 16;
  buffer.set(uuidToBytes(projectId), offset);
  offset += 16;
  buffer.set(uuidToBytes(parentId || '00000000-0000-0000-0000-000000000000'), offset);
  offset += 16;
  view.setInt32(offset, nameBytes.length, true);
  offset += 4;
  buffer.set(nameBytes, offset);
...
export function serializeProjectRenameFolder(
  ...
  let offset = 0;
  buffer[offset] = MessageType.ProjectRenameFolder;
  offset += 1;
  buffer.set(uuidToBytes(messageId), offset);
  offset += 16;
  buffer.set(uuidToBytes(projectId), offset);
  offset += 16;
  buffer.set(uuidToBytes(folderId), offset);
  offset += 16;
  view.setInt32(offset, nameBytes.length, true);
  offset += 4;
  buffer.set(nameBytes, offset);
...
export function deserializeProjectListResult(data: Uint8Array): {
  ...
  for (let i = 0; i < count; i++) {
    ...
    offset += 8;
    const folderCount = view.getInt32(offset, true);
    offset += 4;
...
export function deserializeProjectLoadResult(data: Uint8Array): {
  ...
  if (!success) {
    return { messageId, success, project: null };
  }
  ...
  for (let i = 0; i < folderCount; i++) {
    const folderId = bytesToUuid(data.subarray(offset, offset + 16));
    offset += 16;
    const parentIdRaw = bytesToUuid(data.subarray(offset, offset + 16));
    offset += 16;
    const parentId = parentIdRaw === '00000000-0000-0000-0000-000000000000' ? null : parentIdRaw;
    const folderNameLen = view.getInt32(offset, true);
    offset += 4;
    const folderName = decoder.decode(data.subarray(offset, offset + folderNameLen));
    offset += folderNameLen;
    const isSystem = data[offset] === 1;
    offset += 1;
